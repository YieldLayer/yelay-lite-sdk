...
import { YelayLiteSdk, sdkConfig } from "../src";
...
describe("DepositLockPlugin Tests", function () {
  ...
  before(async function () {
    ...
    sdk = new YelayLiteSdk(impersonatedSigner, sdkConfig[8453]);
    sdkOwner = new YelayLiteSdk(impersonatedDepositLockOwner, sdkConfig[8453]);
  });
  ...
  it("should deposit locked assets successfully", async function () {
    const tx = await sdk.depositLock.depositLocked(
      vaultAddress,
      projectId,
      testAmount
    );
  ...
  it("should redeem locked shares successfully", async function () {
    const tx = await sdk.depositLock.redeemLocked(
      vaultAddress,
      projectId,
      testAmount
    );
  ...
  it("should migrate locked shares between projects", async function () {
    ...
    const tx = await sdk.depositLock.migrateLocked(
      vaultAddress,
      projectId,
      toProjectId,
      testAmount
  ...
  it("should update lock period successfully", async function () {
    const tx = await sdkOwner.depositLock.updateLockPeriod(
      vaultAddress,
      projectId,
      newLockPeriod
    );
  ...
  it("should update global unlock time successfully", async function () {
    const tx = await sdkOwner.depositLock.updateGlobalUnlockTime(
      vaultAddress,
      projectId,
      newUnlockTime
    );
